  ...
  <div class="inventory-adjustment">
    <el-card>
      ...
      <div v-if="!selectedType" class="type-selection">
        ...
        <el-row :gutter="20">
          <el-col
            v-for="(config, type) in selectableMovementTypeConfig"
            :key="type"
            :span="6"
          >
            <el-card
...
import { CREATABLE_MOVEMENT_TYPE_CONFIG, MOVEMENT_TYPE_CONFIG, getMovementTypeLabel as resolveMovementTypeLabel } from '../types'
...
const labels = computed(() => {
  if (localeStore.isEnglish) {
    return {
      ...
      back: 'Back',
      selectType: 'Select adjustment type:',
      currentType: 'Current adjustment type',
      changeType: 'Change',
      product: 'Product',
      ...
      unsupportedType: 'Unsupported adjustment type',
      createSuccess: 'Inventory adjusted',
      createFail: 'Inventory adjustment failed',
  ...
  return {
    ...
    unsupportedType: '不支持的调整类型',
    createSuccess: '库存调整成功',
    createFail: '库存调整失败',
    transferSameWarehouse: '源仓库和目标仓库不能相同',
    transferCreateSuccess: '调拨创建成功',
    transferCreateFail: '创建调拨失败',
...
const selectableMovementTypeConfig = CREATABLE_MOVEMENT_TYPE_CONFIG
...
const rules = computed<FormRules>(() => ({
  product_id: [{ required: true, message: labels.value.productPlaceholder, trigger: 'change' }],
...
const getTypeDescription = (type: MovementType) => {
  if (localeStore.isEnglish) {
    const descriptions: Record<string, string> = {
      ...
      ASSEMBLY_COMPLETE: 'Packing done, pending shipment',
...
const getReferenceTypePlaceholder = () => {
  ...
  const placeholders: Record<MovementType, string> = {
    ...
    PURCHASE_SHIP: localeStore.isEnglish ? 'e.g. Purchase Order' : '如: 采购单',
    WAREHOUSE_RECEIVE: localeStore.isEnglish ? 'e.g. Purchase Order' : '如: 采购单',
    INSPECTION_PASS: localeStore.isEnglish ? 'e.g. Inspection Sheet' : '如: 质检单',
    INSPECTION_FAIL: localeStore.isEnglish ? 'e.g. Inspection Sheet' : '如: 质检单',
    ASSEMBLY_COMPLETE: localeStore.isEnglish ? 'e.g. Packing Order' : '如: 打包单',
    RETURN_INSPECT: localeStore.isEnglish ? 'e.g. Return Order' : '如: 退货单'
  }
